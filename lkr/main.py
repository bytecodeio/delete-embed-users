...
def delete_embed_users(
    ...
    # page_size controls how many users we request per search_users call.
    ...
    while not finished:
        ...
        with ThreadPoolExecutor(max_workers=batch_size) as executor:
            ...
            for future in futures:
                ...
                # collect up to the overall --limit for deletion
                if limit and len(users_to_delete) < limit:
                    remaining = limit - len(users_to_delete)
                    users_to_delete.extend(response[:remaining])
    ...
    # Dry-run: show the total number of matching users and how many would be deleted (capped)
    if dry_run:
        typer.echo(f"Found {total_matches} matching users (will delete up to {len(users_to_delete)})")
        return
    else:
        typer.echo(f"Deleting {len(users_to_delete)} users (out of {total_matches} matches)")
        with ThreadPoolExecutor(max_workers=batch_size) as executor:
            future_to_user = {
                executor.submit(sdk.delete_user, user.id): user
            ...
            for future in as_completed(future_to_user):
                ...
                try:
                    ...
                    typer.echo(f"Error deleting user {user.id}: {e}")
